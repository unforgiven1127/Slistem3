...
$oMyCx = mysqli_init();
if(!$oMyCx->real_connect('localhost', 'pma_admin', 'KVW4PVVAWHASuRDz'))
{
  echo $oMyCx->error();
...
INNER JOIN cons_tbl as co ON (co.cons_cid = ca.candi_cid)
...
`title` , `department` , `grade` , `salary` , `bonus` */
  ...
  $bInserted = mysqli_query($oMyCx, $sMyQuery);
  if(!$bInserted)
  {
    echo mysqli_error($oMyCx);
    var_dump($sMyQuery);
    exit('<br /><br />error inserting candidates during pass #'.$nPass.' / offset: '.$nLimitOffset);
  }
  ...
  if(!$bInserted)
  {
    echo mysqli_error($oMyCx);
    var_dump($sMyQuery);
    exit('<br /><br />error inserting candidates profile during pass #'.$nPass.' / offset: '.$nLimitOffset);
  }
