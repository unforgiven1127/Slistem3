...
while(!$bError && !$bDone && $nPass < $nMaxPass)
{
  ...
  $sPgQuery = 'SELECT * FROM language_tbl ORDER BY lang_id ASC
  ...
  while($asData = pg_fetch_assoc($oPgResult))
  {
    ...
    $asData['lang_id'] = (int)$asData['lang_id'];
    if($asData['lang_id'] > 0)
    {
      $asData['lang_name'] = '"'.mysql_real_escape_string(addslashes(trim($asData['lang_name']))).'"';

      //(`eventpk`,`type`,`title`,`content`,`date_create`,`date_display`,`created_by`)
      $asMyInsert[] = '('.$asData['lang_id'].' , '.$asData['lang_name'].')';
    }
  }
  ...
  $sMyQuery = 'INSERT INTO `sl_language` (`sl_languagepk`,`language`) VALUES '.implode(' ,', $asMyInsert);
  $bInserted = mysql_query($sMyQuery);
  if(!$bInserted)
  {
    echo mysql_error();
    var_dump($sMyQuery);
    exit('error inserting languages during pass #'.$nPass.' / offset: '.$nLimitOffset);
  }
