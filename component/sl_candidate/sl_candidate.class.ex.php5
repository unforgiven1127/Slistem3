  ...
  //====================================================================
    ...
    public function logAjax()
    {
      ...
      if($logType == "candiTab2")
      {
        ...
        $return = securityCheckContactView($user_id);
      }
...
//bunu ekleyelim contact work phone
      ...
      if(isset($sSortField) && !empty($sSortField) && $sSortField == '_in_play')
      {
        ...
        if($sSortOrder == 'asc')
        {
          ...
          $poQB->addSelect('IFNULL(slpl.status,0) + (IFNULL(slpl.active,9999) * 1000) + (IFNULL(slpl3.active,0)*10000) as statusSort');
        }
        else
        {
          $poQB->addSelect('IFNULL(slpl.status,0) as candidateLatestStatus');
      ...
      else if(isset($sSortField) && !empty($sSortField) && $sSortField == "salary")
      {
        if($sSortOrder == 'asc')
        {
          $poQB->addSelect('case (scpr.salary + scpr.bonus) WHEN 0 THEN 99999999999999999999999 ELSE (scpr.salary + scpr.bonus) END as salarySort');
      ...
      switch($sTemplate)
      {
        ...
        default:
          ...
          if($bDisplayPositionField)
          {
            ...
            $sTitleW = '10%'; /*$sDeptW = '9%';*/ $sDeptW ='140';
          ...
          else
          {
            ...
            $sTitleW = '11%'; /*$sDeptW = '10%';*/ $sDeptW ='140';
          ...
          if(in_array('phone', $this->casSettings['candi_list_field']))
            $oConf->addColumn('Work Phone', 'phone', array('id' => '', 'width' => '80',
              'sortable'=> array($sSortJs => 'value_integer', 'ajax' => $nAjax, 'url' => $sURL, 'ajax_target' => $this->csSearchId)));
...
*/
    ...
    private function _getMeetingDoneForm($pnCandiPk, $pnMeetingPk)
    {
      ...
      else
      {
        ...
        $data['status_options'] = '<option value="0"> - </option>
...
ChromePhp::log($array);
      ...
      if(!$characterNoteControlFlag)
      {
        ...
        return array('error' => __LINE__.' - Warning! Please add character notes to continue');
...
die();*/
    ...
    private function _saveCandidate($pnCandidatePk = 0)
    {
      ...
      if(!empty($pnCandidatePk))
      {
        ...
        if(!$this->_oLogin->isAdmin() && htmlspecialchars_decode($asData['firstname'] , ENT_QUOTES) != htmlspecialchars_decode(getValue('firstname'), ENT_QUOTES))
        ...
        if(!$this->_oLogin->isAdmin() && htmlspecialchars_decode($asData['lastname'] , ENT_QUOTES) != htmlspecialchars_decode(getValue('lastname'), ENT_QUOTES))
          return array('popupError' => 'Normal user cannot change candidate name');
...
$bonusManual = getValue('bonus');
      ...
      if($pbSave)
      {
        ...
        else
        {
          ...
          if(isset($pasCandidate['companyfk']) && $asData['companyfk'] != $nNewCompanyFk)
          {
...
// candidate duplica control ends
  ...
  private function _addOldCompany($candidate_id,$company_id)
  {
    ...
    private function _getNoScoutList()
    {
      ...
      {
        $sQuery = 'SELECT * FROM sl_company WHERE length(name) > 1 AND  level = '.$nLevel.' ORDER BY name ASC';
      }
      else if(!empty($nLevel))
      {// parantez icinde OR is_nc_ok = 0 vardi kaldirdik
        $sQuery = 'SELECT * FROM sl_company WHERE length(name) > 1 AND  level = '.$nLevel.' AND (is_client = 1) ORDER BY name ASC';
      }
      else // OR is_nc_ok = 0 vardi kaldirdik
      {
        $sQuery = 'SELECT * FROM sl_company WHERE length(name) > 1 AND  level in(1,2,3) AND is_client = 1  ORDER BY name ASC';
      }
      ...
      while($bRead)
      {
        ...
        $sCompany = '<div class="cp_ns_row">
            ...
            <div class="plist"><a href="javascript:;" onclick="popup_candi(this, \''.$positionsURL.'\',1050,700);" style="cursor: pointer;">Position list</a></div>
