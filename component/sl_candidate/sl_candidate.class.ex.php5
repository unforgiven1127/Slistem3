...
//ChromePhp::log($asError);
    ...
    private function _getNoScoutList()
    {
      ...
      {// parantez icinde OR is_nc_ok = 0 vardi kaldirdik
        $sQuery = 'SELECT * FROM sl_company WHERE LEN(name) > 3 AND level = '.$nLevel.' AND (is_client = 1) ORDER BY name ASC';
      }
      else // OR is_nc_ok = 0 vardi kaldirdik
      {
        $sQuery = 'SELECT * FROM sl_company WHERE LEN(name) > 3 AND level in(1,2,3) AND is_client = 1  ORDER BY name ASC';
      }
