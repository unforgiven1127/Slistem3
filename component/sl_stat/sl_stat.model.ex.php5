 ...
 */
  ...
  $query = 'SELECT m.*, min(m2.sl_meetingpk) as min_date, slc._sys_status as candidate_status
        ...
        INNER JOIN sl_meeting m2 on m2.candidatefk = m.candidatefk and m2.meeting_done = 1
        INNER JOIN sl_candidate slc on slc.sl_candidatepk = m.candidatefk AND slc._sys_status = 0
        WHERE (m.created_by IN '.$user_ids.' OR m.attendeefk IN '.$user_ids.')
        AND m.date_met >= "'.$start_date.'"
        AND m.date_met < "'.$end_date.'"
        group by m.sl_meetingpk
        order by m.candidatefk';
  ...
  public function get_revenue_data($request_date = '', $location = '')
  {
    ...
    if ($read)
    {
      ...
      $query = "SELECT l.*, sln.shortname as nationality
                ...
                WHERE (l.position = 'Researcher' OR l.loginpk = '457' OR l.loginpk = '481' OR l.loginpk = '466' OR l.loginpk = '480') AND l.loginpk != '382'"; // saruul cici vs hem consultant hem researcher da gorunebilmesi icin...
...
//var_dump('<br><br>');
      ...
      while($read)
      {
        ...
        if($user_id == '457' || $user_id == '481' || $user_id == '466'  || $user_id == '480')
...
*/
  ...
  public function newKPImetInfo($user_type = 'consultant', $start_date, $end_date, $user_list)
  {
    ...
    else
    {
      ...
      FROM sl_meeting slm
      LEFT JOIN login l ON l.loginpk = slm.created_by
      WHERE slm.date_met >= '".$start_date."' AND slm.date_met <= '".$end_date."' AND slm.created_by in ".$user_list. " AND meeting_done = '1' GROUP BY slm.created_by";
    }
  ...
  public function newKPIccm1setInfo($user_type = 'consultant', $start_date, $end_date, $user_list)
  {
    if($user_type == 'consultant')
    {
    ...
    else
    {
      ...
      LEFT JOIN sl_meeting slm ON slm.candidatefk = slpl.candidatefk
      LEFT JOIN login l ON l.loginpk = slm.created_by
      WHERE slpl.date_created >= '".$start_date."' AND slpl.date_created <= '".$end_date."' AND slpl.status = 51 AND slm.created_by in ".$user_list." GROUP BY slpl.candidatefk, slpl.positionfk";
    }
  ...
  public function newKPIccm1doneInfo($user_type = 'consultant', $start_date, $end_date, $user_list)
  {
    if($user_type == 'consultant')
    {
      $query = "SELECT l.loginpk as user_id, l.firstname, l.lastname,slpl.positionfk, COUNT(slpl.sl_position_linkpk) as ccm1done_count, GROUP_CONCAT(slpl.candidatefk SEPARATOR ', ') as candidates
      FROM sl_position_link slpl
      LEFT JOIN login l ON l.loginpk = slpl.created_by
      WHERE slpl.date_completed >= '".$start_date."' AND slpl.date_completed <= '".$end_date."' AND slpl.status = 51 AND slpl.active = 0 AND slpl.created_by in ".$user_list." GROUP BY slpl.candidatefk, slpl.positionfk";
    }
    else
    {
      $query = "SELECT l.loginpk as user_id, l.firstname, l.lastname,slpl.positionfk, COUNT(slpl.sl_position_linkpk) as ccm1done_count, GROUP_CONCAT(slpl.candidatefk SEPARATOR ', ') as candidates
      ...
      LEFT JOIN sl_meeting slm ON slm.candidatefk = slpl.candidatefk
      LEFT JOIN login l ON l.loginpk = slm.created_by
      WHERE slpl.date_completed >= '".$start_date."' AND slpl.date_completed <= '".$end_date."' AND slpl.status = 51 AND slpl.active = 0 AND slm.created_by in ".$user_list." GROUP BY slpl.candidatefk, slpl.positionfk";
    }
  ...
  public function newKPIccm2setInfo($user_type = 'consultant', $start_date, $end_date, $user_list)
  {
    if($user_type == 'consultant')
    {
      $query = "SELECT l.loginpk as user_id, l.firstname, l.lastname,slpl.positionfk, COUNT(slpl.sl_position_linkpk) as ccm2set_count, GROUP_CONCAT(slpl.candidatefk SEPARATOR ', ') as candidates
      FROM sl_position_link slpl
      LEFT JOIN login l ON l.loginpk = slpl.created_by
    ...
    else
    {
      ...
      LEFT JOIN sl_meeting slm ON slm.candidatefk = slpl.candidatefk
      LEFT JOIN login l ON l.loginpk = slm.created_by
      WHERE slpl.date_created >= '".$start_date."' AND slpl.date_created <= '".$end_date."' AND slpl.status = 52 AND slm.created_by in ".$user_list." GROUP BY slpl.candidatefk,slpl.positionfk";
    }
  ...
  public function newKPIccm2doneInfo($user_type = 'consultant', $start_date, $end_date, $user_list)
  {
    if($user_type == 'consultant')
    {
      $query = "SELECT l.loginpk as user_id, l.firstname, l.lastname,slpl.positionfk, COUNT(slpl.sl_position_linkpk) as ccm2done_count, GROUP_CONCAT(slpl.candidatefk SEPARATOR ', ') as candidates
      FROM sl_position_link slpl
      LEFT JOIN login l ON l.loginpk = slpl.created_by
    ...
    else
    {
      ...
      LEFT JOIN sl_meeting slm ON slm.candidatefk = slpl.candidatefk
      LEFT JOIN login l ON l.loginpk = slm.created_by
      WHERE slpl.date_completed >= '".$start_date."' AND slpl.date_completed <= '".$end_date."' AND slpl.status = 52 AND slpl.active = 0 AND slm.created_by in ".$user_list." GROUP BY slpl.candidatefk, slpl.positionfk";
    }
  ...
  public function newKPImccmsetInfo($user_type = 'consultant', $start_date, $end_date, $user_list)
  {
    ...
    else
    {
      ...
      LEFT JOIN sl_meeting slm ON slm.candidatefk = slpl.candidatefk
      LEFT JOIN login l ON l.loginpk = slm.created_by
      WHERE slpl.date_created >= '".$start_date."' AND slpl.date_created <= '".$end_date."' AND slpl.status > 52 AND slpl.status < 70 AND slm.created_by in ".$user_list." GROUP BY slpl.candidatefk, slpl.positionfk";
    }
  ...
  public function newKPImccmdoneInfo($user_type = 'consultant', $start_date, $end_date, $user_list)
  {
    if($user_type == 'consultant')
    {
      $query = "SELECT l.loginpk as user_id, l.firstname, l.lastname,slpl.positionfk, COUNT(slpl.sl_position_linkpk) as mccmdone_count, GROUP_CONCAT(slpl.candidatefk SEPARATOR ', ') as candidates
      FROM sl_position_link slpl
      LEFT JOIN login l ON l.loginpk = slpl.created_by
      WHERE slpl.date_completed >= '".$start_date."' AND slpl.date_completed <= '".$end_date."' AND slpl.status > 52 AND slpl.status < 70 AND slpl.active = 0 AND slpl.created_by in ".$user_list." GROUP BY slpl.candidatefk, slpl.positionfk";
    }
    else
    {
      $query = "SELECT l.loginpk as user_id, l.firstname, l.lastname,slpl.positionfk, COUNT(slpl.sl_position_linkpk) as mccmdone_count, GROUP_CONCAT(slpl.candidatefk SEPARATOR ', ') as candidates
      ...
      LEFT JOIN sl_meeting slm ON slm.candidatefk = slpl.candidatefk
      LEFT JOIN login l ON l.loginpk = slm.created_by
      WHERE slpl.date_completed >= '".$start_date."' AND slpl.date_completed <= '".$end_date."' AND slpl.status > 52 AND slpl.status < 70 AND slpl.active = 0 AND slm.created_by in ".$user_list." GROUP BY slpl.candidatefk, slpl.positionfk";
    }
...
//ChromePhp::log($query);
  ...
  public function newKPIplacedInfo($user_type = 'consultant', $start_date, $end_date, $user_list)
  {
    if($user_type == 'consultant')
    {
