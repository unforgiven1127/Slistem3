...
//var_dump('<br><br>');
      ...
      while($read)
      {
        ...
        $researcherPlacementQuery = "SELECT count(rm.loginpk) as count FROM revenue r INNER JOIN revenue_member rm on rm.revenue_id = r.id and rm.loginpk = '".$user_id."' AND r.salary > 0 AND rm.user_position = 'Researcher' WHERE r.date_due >= '".$date_start."' AND r.date_due <= '".$date_end."'";
