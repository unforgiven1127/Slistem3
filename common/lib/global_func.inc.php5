...
//ChromePhp::log($sQuery);
  ...
  function storeLastSearch($lastSearch,$sSortField,$sSortOrder, $user_id)
  {
      $sDate = date('Y-m-d H:i:s');
      $oDB = CDependency::getComponentByName('database');

      $sQuery = "INSERT INTO `last_searches` (`user_id`,`query`,`sort_field`,`sort_order`,`date`) VALUES('".$user_id."','".$lastSearch."','".$sSortField."','".$sSortOrder."','".$sDate."')";
  ...
  {
    $oDB = CDependency::getComponentByName('database');
    $sQuery = "SELECT * FROM sl_generated_kpi gk WHERE gk.type = 'kpi' AND gk.created_date = '".$date."' ORDER BY gk.id ASC LIMIT 1";
